...
# =============================================================================
# 13. T12 — YIELD-OPTIMIZED 3-BAND CONTROL
# =============================================================================
#
# This section is added after completion of the original 10 representative
# triplets.  The new run SKIPS those 10 triplets entirely and performs T12 only.
#
# T12 answers:
#   "If the only goal is wheat-yield prediction, why not use the measured HSI
#    to choose the three most yield-informative wavelengths directly?"
#
# Selection uses ONLY the frozen yield TRAIN + VALIDATION sets.  The yield TEST
# set is never evaluated during band selection.  Once the final three bands are
# frozen, the normal full Experiment-2 evaluation is run on them:
#
#   selected 3 bands -> yield                  (10 seeds)
#   selected 3 bands -> fixed 3->128->128->40 MLP
#   reconstructed40 -> yield                  (10 seeds)
#   held-out reconstruction SAM/RMSE/MAE
#
# The original Experiment-2 reconstruction model, optimizer, stopping rule,
# masks, splits, feature definitions, and yield network are reused unchanged.
# =============================================================================

T12_OUT = OUT / "t12_yield_optimized_control"

# Fixed seeds used only while SEARCHING for the T12 bands.
# Final evaluation still uses YIELD_MODEL_SEEDS = 0..9 from Experiment 2.
# If a quick pilot is ever needed, this can temporarily be changed to (0,),
# but the final manuscript run should keep the fixed setting below.
T12_SELECTION_SEEDS = (0, 1, 2)


def t12_measured_features_for_bands(X_measured40, bands):
    """Construct direct sparse-band yield features from measured40 features.

    measured40 feature layout from Experiment 2:
        0:40   = subplot per-band means
        40:80  = subplot per-band standard deviations
        80     = SL pixel area

    For k selected bands this returns:
        k means + k standard deviations + area
    """

    bands = tuple(sorted(int(i) for i in bands))

    return np.concatenate(
        [
            X_measured40[:, list(bands)],
            X_measured40[:, [40 + i for i in bands]],
            X_measured40[:, 80:81],
        ],
        axis=1,
...
def t12_validation_score(
    ...
    """Score one candidate band set using validation RMSE only.

    IMPORTANT: the yield TEST set is not passed to this function and therefore
    cannot affect T12 selection.
    """

    Z, _, _ = standardize_from_train(X, train_idx)
    ...
    for model_seed in seeds:
        ...
        rmse_values.append(float(math.sqrt(best_val_mse)))
    ...
    return {
        "selection_val_rmse_mean": float(np.mean(rmse_values)),
        "selection_val_rmse_sd": float(np.std(rmse_values, ddof=0)),
        "selection_val_rmse_by_seed": rmse_values,
        "best_epoch_by_seed": best_epochs,
    }


def greedy_t12_selection(
    X_measured40,
    y,
    yield_train_idx,
    yield_val_idx,
    ...
    """Three-step greedy forward selection using yield validation RMSE."""

    T12_OUT.mkdir(parents=True, exist_ok=True)

    selected = []

    step_files = [
        T12_OUT / "t12_step1_single_band.csv",
        T12_OUT / "t12_step2_pair.csv",
        T12_OUT / "t12_step3_triplet.csv",
    ]

    print("\n" + "=" * 100)
    print("T12 — GREEDY YIELD-OPTIMIZED BAND SELECTION")
    print("=" * 100)
    print("Selection seeds:", T12_SELECTION_SEEDS)
    print("Selection criterion: mean best VALIDATION RMSE")
    print("Yield TEST labels are NOT used during T12 selection.")

    for step in range(1, 4):
        step_path = step_files[step - 1]
        expected_n = 40 - (step - 1)

        # Safe resume: if a complete ranking for this step already exists,
        # reuse it and continue directly to the next step.
        if step_path.exists():
            cached = pd.read_csv(step_path)

            if (
                len(cached) == expected_n
                and "selection_val_rmse_mean" in cached.columns
                and "band_indices" in cached.columns
            ):
                ranked = cached.sort_values(
                    "selection_val_rmse_mean",
                    ascending=True,
                ...
                selected = [
                    int(v)
                    for v in str(best["band_indices"]).split("-")
                ]

                print("\nT12 step {}: using existing complete ranking".format(step))
                print("  selected indices =", selected)
                print(
                    "  selected nm      =",
                    tuple(
                        round(float(wavelengths40[i]), 3)
                        for i in selected
                    ),
                )
                print(
                    "  validation RMSE  = {:.5f}".format(
                        float(best["selection_val_rmse_mean"])
                    )
                )
                continue

        rows = []
        remaining = [i for i in range(40) if i not in selected]

        print("\nT12 step {}/3".format(step))
        print("  fixed previous indices =", selected)
        print("  candidates             =", len(remaining))

        for candidate_no, band in enumerate(remaining, start=1):
            bands = tuple(sorted(selected + [band]))

            X = t12_measured_features_for_bands(
                X_measured40,
                bands,
            )

            score = t12_validation_score(
                X=X,
                y=y,
                train_idx=yield_train_idx,
                val_idx=yield_val_idx,
                device=device,
                seeds=T12_SELECTION_SEEDS,
            )

            row = {
                "step": int(step),
                "band_indices": "-".join(str(i) for i in bands),
                "wl_nm": "-".join(
                    "{:.4f}".format(float(wavelengths40[i]))
                    for i in bands
                ),
                "added_band_idx": int(band),
                "added_wavelength_nm": float(wavelengths40[band]),
                "selection_val_rmse_mean": score[
                    "selection_val_rmse_mean"
                ],
                "selection_val_rmse_sd": score[
                    "selection_val_rmse_sd"
                ],
                "selection_val_rmse_by_seed": json.dumps(
                    score["selection_val_rmse_by_seed"]
                ),
                "best_epoch_by_seed": json.dumps(
                    score["best_epoch_by_seed"]
                ),
            }
            ...
            print(
                "  {:2d}/{:2d} | idx {} | nm {} | val RMSE {:.5f} +/- {:.5f}".format(
                    candidate_no,
                    len(remaining),
                    bands,
                    tuple(
                        round(float(wavelengths40[i]), 1)
                        for i in bands
                    ),
                    score["selection_val_rmse_mean"],
                    score["selection_val_rmse_sd"],
                )
            )

            # Incremental save.  An interrupted step is simply recomputed on
            # the next run; a completed step is reused automatically.
            pd.DataFrame(rows).to_csv(step_path, index=False)
        ...
        ranked.to_csv(step_path, index=False)
        ...
        print(
            "  mean validation RMSE = {:.5f}".format(
                float(best["selection_val_rmse_mean"])
            )
        )
    ...
    payload = {
        "selection_name": "T12_yield_optimized",
        "selection_method": "three-step greedy forward selection",
        "selection_metric": "mean best validation RMSE",
        "selection_seeds": list(T12_SELECTION_SEEDS),
        "yield_test_used_for_selection": False,
        "selected_band_indices_40": list(selected),
        "selected_wavelengths_nm": [
            float(wavelengths40[i])
            for i in selected
        ],
    }

    with open(T12_OUT / "t12_selected.json", "w") as f:
...
def final_evaluate_t12(
    ...
    """Run the exact Experiment-2 reconstruction/yield protocol on frozen T12."""

    selected_idx = tuple(sorted(int(i) for i in selected_idx))
    actual = tuple(float(wavelengths40[i]) for i in selected_idx)

    # Include selected indices in the directory name so stale features from a
    # different T12 selection can never be reused accidentally.
    final_dir = T12_OUT / "final_evaluation_idx_{}_{}_{}".format(
        selected_idx[0], selected_idx[1], selected_idx[2]
    )
    final_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 100)
    print("FINAL T12 EVALUATION")
    print("=" * 100)
    print("indices =", selected_idx)
    print("nm      =", tuple(round(v, 3) for v in actual))

    # A. Same fixed reconstruction model/settings as all 10 Experiment-2 runs.
    print("\n[A] Train fixed direct 3 -> 40 MLP")
    ...
    # B. Same frozen features and held-out reconstruction metrics.
    ...
    # C. Full 10-seed direct three-band yield evaluation.
    print("\n[C] T12 selected 3 bands -> yield")

    direct_summary = train_yield_representation(
        representation="T12_direct_sparse3",
        X=X3,
        y=y,
        train_idx=yield_train_idx,
        val_idx=yield_val_idx,
        ...
        device=device,
        out_dir=final_dir / "yield_direct_sparse3",
    )

    # D. Full 10-seed reconstructed40 yield evaluation.
    print("\n[D] T12 reconstructed40 -> yield")

    recon_summary = train_yield_representation(
        representation="T12_reconstructed40",
        X=Xrec,
        y=y,
        train_idx=yield_train_idx,
        val_idx=yield_val_idx,
        test_idx=yield_test_idx,
        device=device,
        out_dir=final_dir / "yield_reconstructed40",
    )

    # If the completed original Experiment-2 table exists, copy its fixed
    # measured40 baseline values into the T12 row for column compatibility.
    measured_r2 = np.nan
    measured_rmse = np.nan
    old_results_path = OUT / "experiment2_results.csv"

    if old_results_path.exists():
        old_results = pd.read_csv(old_results_path)
        if len(old_results) > 0:
            if "measured40_yield_r2" in old_results.columns:
                measured_r2 = float(old_results.iloc[0]["measured40_yield_r2"])
            if "measured40_yield_rmse" in old_results.columns:
                measured_rmse = float(old_results.iloc[0]["measured40_yield_rmse"])

    row = {
        "triplet_name": "T12_yield_optimized",
        "nominal_wl1": actual[0],
        "nominal_wl2": actual[1],
        "nominal_wl3": actual[2],
        ...
        "recon_best_epoch": int(best_epoch),
        "recon_best_val_mse_scaled": float(best_val_mse),
        "recon_test_sam_deg": float(recon_metrics["sam_deg"]),
        "recon_test_rmse": float(recon_metrics["rmse"]),
        "recon_test_mae": float(recon_metrics["mae"]),
        "recon_test_pixels": int(recon_metrics["test_pixels"]),

        "direct_primary_seed": int(direct_summary["primary_seed"]),
        "direct_yield_r2": float(direct_summary["primary_test_R2"]),
        "direct_yield_rmse": float(direct_summary["primary_test_RMSE"]),
        "direct_yield_mae": float(direct_summary["primary_test_MAE"]),
        "direct_yield_r2_mean10": float(direct_summary["mean_test_R2"]),
        "direct_yield_r2_sd10": float(direct_summary["sd_test_R2"]),

        "reconstructed_primary_seed": int(recon_summary["primary_seed"]),
        "reconstructed_yield_r2": float(recon_summary["primary_test_R2"]),
        "reconstructed_yield_rmse": float(recon_summary["primary_test_RMSE"]),
        "reconstructed_yield_mae": float(recon_summary["primary_test_MAE"]),
        "reconstructed_yield_r2_mean10": float(recon_summary["mean_test_R2"]),
        "reconstructed_yield_r2_sd10": float(recon_summary["sd_test_R2"]),

        "measured40_yield_r2": measured_r2,
        "measured40_yield_rmse": measured_rmse,
    }

    pd.DataFrame([row]).to_csv(
        T12_OUT / "t12_final_result.csv",
        index=False,
    )

    with open(T12_OUT / "t12_final_result.json", "w") as f:
        json.dump(row, f, indent=2)

    print("\n" + "=" * 100)
    print("FINAL T12 RESULT")
    print("=" * 100)
    print("selected wavelengths      =", tuple(round(v, 3) for v in actual))
    print("direct sparse3 yield R2   = {:.4f}".format(row["direct_yield_r2"]))
    print("reconstruction test SAM  = {:.4f} deg".format(row["recon_test_sam_deg"]))
    print("reconstruction test RMSE = {:.6f}".format(row["recon_test_rmse"]))
    print("reconstructed40 yield R2 = {:.4f}".format(row["reconstructed_yield_r2"]))
...
def main():
    ...
    print("=" * 100)
    print("HSIwheat EXPERIMENT 2 — T12-ONLY ADDITION")
    print("=" * 100)
    print("Device:", device)
    print("Root:", ROOT)
    print("\nIMPORTANT:")
    print("  Original 10 representative triplets are SKIPPED in this run.")
    print("  Measured40 yield baseline is NOT retrained.")
    print("  This run performs only T12 selection + final T12 evaluation.")
    print("  Existing Experiment-2 caches/splits are reused when available.")

    if not WAVELENGTHS40_PATH.exists():
        raise FileNotFoundError("Missing {}".format(WAVELENGTHS40_PATH))
    ...
    # ------------------------------------------------------------------
    # Reuse the exact frozen Experiment-2 data/splits/caches.
    # ------------------------------------------------------------------
    ...
    # Same existence check as original Experiment 2.
    for field, plot in (
        meta[["field", "plot"]]
        .drop_duplicates()
    ...
    print(
        "  test       =",
        "{:,}".format(sum(end - start for start, end in test_ranges)),
    )
    ...
    # Reuses Experiment-2 cache when it exists; otherwise extracts it once.
    # No measured40 yield model is trained in this T12-only run.
    ...
    # ------------------------------------------------------------------
    # T12 selection: TRAIN + VALIDATION only.  No test labels touched.
    # ------------------------------------------------------------------
    t12_idx = greedy_t12_selection(
        X_measured40=X_measured40,
        y=y,
        yield_train_idx=yield_train_idx,
        yield_val_idx=yield_val_idx,
        wavelengths40=wavelengths40,
        device=device,
    )

    # ------------------------------------------------------------------
    # T12 is now frozen.  Run the exact full Experiment-2 evaluation once.
    # ------------------------------------------------------------------
    t12_row = final_evaluate_t12(
        selected_idx=t12_idx,
        wavelengths40=wavelengths40,
        meta=meta,
        y=y,
        yield_train_idx=yield_train_idx,
        yield_val_idx=yield_val_idx,
        yield_test_idx=yield_test_idx,
        measured_maps=measured_maps,
        recon_split_df=recon_split_df,
        pixel_matrix=pixel_matrix,
        train_indices=train_indices,
        val_indices=val_indices,
        mean40=mean40,
    ...
    # ------------------------------------------------------------------
    # Append T12 to the already-completed 10-triplet Experiment-2 table.
    # The old file is NEVER overwritten.
    # ------------------------------------------------------------------
    old_path = OUT / "experiment2_results.csv"

    if old_path.exists():
        old = pd.read_csv(old_path)

        # If this script is rerun, avoid duplicating an existing T12 row.
        if "triplet_name" in old.columns:
            old = old[old["triplet_name"] != "T12_yield_optimized"].copy()

        combined = pd.concat(
            [old, pd.DataFrame([t12_row])],
            ignore_index=True,
            sort=False,
        )

        combined_path = T12_OUT / "experiment2_plus_t12.csv"
        ...
        if all(c in combined.columns for c in relation_cols):
            combined[relation_cols].corr(method="pearson").to_csv(
                T12_OUT / "experiment2_plus_t12_pearson.csv"
            )
            combined[relation_cols].corr(method="spearman").to_csv(
                T12_OUT / "experiment2_plus_t12_spearman.csv"
            )

        print("\nCombined original Experiment 2 + T12:")
        print(" ", combined_path)
    else:
        print("\nNOTE: original experiment2_results.csv was not found.")
        print("T12 result was still saved normally, but no combined table was created.")

    print("\nSaved T12 outputs under:")
    print(" ", T12_OUT)
    print(" ", T12_OUT / "t12_selected.json")
    print(" ", T12_OUT / "t12_final_result.csv")

    print(
        "\nTotal T12-only elapsed: {:.2f} h".format(
            (time.time() - t0) / 3600.0
        )
    )
